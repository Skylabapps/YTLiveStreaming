...
extension YTLiveStreaming {
    ...
    public func createBroadcast(_ title: String,broadCasteType:String, description: String?, startTime: Date, completion: @escaping (Error?,LiveBroadcastStreamModel?) -> Void) {
        
        // Create Live broadcast
        let liveStreamDescription = description == nil ? "" : description!
        let liveStreamName = "SkyLab"
        
        YTLiveRequest.createLiveBroadcast(title, broadCasteType:broadCasteType , startDateTime: startTime, completion: { error, liveBroadcastModel in
                    ...
                    if let liveStream = liveStream {
                        ...
                        YTLiveRequest.bindLiveBroadcast(broadcastId: liveBroadcast.id, liveStreamId: liveStream.id, completion: { liveBroadcast in
                            if let liveBroadcast = liveBroadcast {
                                completion(error, liveBroadcast)
                            } else {
    ...
    public func completeBroadcast(_ broadcast: String, completion: @escaping (Bool) -> Void) {
        LiveLauncher.sharedInstance.stopBroadcast()
        // complete – The broadcast is over. YouTube stops transmitting video.
        YTLiveRequest.transitionLiveBroadcast(broadcast, broadcastStatus: "complete", completion: { liveBroadcast in
